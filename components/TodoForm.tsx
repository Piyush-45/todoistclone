...
import { mmkvstorage } from '@/app/(authenticated)/task/date-selector';
  ...
  const TodoForm = ({ todo,drizzleDb }: TodoFormProps) => {
   ...
   // **Handle date updates from the date picker screen
  ...
  useEffect(() => {
    if (previouslySelectedDate) {
      ...
      console.log(mmkvstorage.getString("name"))
    }
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    if (params.selectedDate) {
        setSelectedDate(new Date(params.selectedDate as string));
    }
}, [params.selectedDate]);
        ...
        <View style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <View style={styles.centeredView}>
              <View style={styles.modalView}>
                <FlatList
                  ...
                  ListHeaderComponent={()=><Button title='cancel' onPress={()=>setShowProjects(false)}/>}
          ...
          <ScrollView contentContainerStyle={[styles.container]} keyboardShouldPersistTaps="always">
            ...
            <ScrollView
             ...
             <Pressable
                onPress={() => {
                    // Pass the current date as a parameter to the date select screen
                    router.push({
                        pathname: '/task/date-selector',
                        params: { currentDate: selectedDate.toISOString() }
                    });
                }}
                style={({ pressed }) => {
                    return [
